...
def transform_data(sources, destination_bucket):
    dfPCObraInco = sources["t_paco_registro_obras_inconclusa"]
    dfPrCon = sources["t_seii_procecotrata_compraadjudi"]
    date_data = date.today()

    dfFinal = dfPrCon.dropDuplicates(["id_portafolio"]).join(dfPCObraInco.select("nombre_rol_1").filter(~col("nombre_rol_1").isNull()).distinct(),col("nombre_proveedor")==col("nombre_rol_1"), "inner")

    df_result = dfFinal.agg(
        lit("indicadores por inhabilidad").cast("string").alias("nombre_grupo_indicador"),
        lit("inhabilitados por obras inconclusas").cast("string").alias("nombre_indicador"),
