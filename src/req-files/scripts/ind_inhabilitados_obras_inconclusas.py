...
def transform_data(sources, destination_bucket):
    ...
    df42 = dfPrCon.dropDuplicates(["id_portafolio"]).join(
        dfPCObraInco.select("nombre_rol_1").filter(~col("nombre_rol_1").isNull()).distinct(),
        col("nombre_proveedor") == col("nombre_rol_1"), "inner")
