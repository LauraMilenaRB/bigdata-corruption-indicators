...
def transform_data(spark, key, date_origin, source_bucket, destination_bucket):
    ...
    data_source = spark.read.option("header", True).csv(f"s3://{source_bucket}/origen/SECOP_II_-_Procesos_de_Contrataci_n.csv")
    ...
    data_transform = data_source.select(
        ...
        trim(col("Departamento Entidad")).alias("nombre_depto_entidad"),
        ...
        trim(col("Justificación Modalidad de Contratación")).alias("desc_modalidad_contrato"),
        ...
        upper(trim(col("Nombre del Proveedor Adjudicado"))).alias("nombre_proveedor"),
        ...
        trim(col("Estado Resumen")).alias("tipo_estado_resumen"),
